...
export async function performTrustCheck(
  ...
  // 2. Get AI summary by passing the entire structured analysis object
  const summary = await summarizeTrustCheckResults(analysis);
...
- Website Content: ${analysis.websiteContent?.error ? `Error: ${analysis.websiteContent.error}` : (analysis.websiteContent?.summary ? `Summary: ${analysis.websiteContent.summary}` : 'N/A')}
